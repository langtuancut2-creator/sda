import srtToAss from "./server/utils/srt-to-ass.js";
...
const uploadVideo = multer({ dest: path.join(os.tmpdir(), 'upload_video'), limits: { fileSize: 2000 * 1024 * 1024 } });
  ...
  if (videoFile.size > MAX_VIDEO_BYTES) {
    try { rmSync(videoFile.path); } catch(e){}
  ...
  const tmpBase = mkdtempSync(path.join(os.tmpdir(), 'sda-export-capcut-'));
  try {
    ...
    renameSync(videoFile.path, inputPath);
    ...
    if (fontFile) {
      ...
      renameSync(fontFile.path, fontPath);
    }

    if (subtitleFile) {
      const srtPath = path.join(tmpBase, 'subtitle.srt');
      renameSync(subtitleFile.path, srtPath);
    ...
    if (logoFile) {
      ...
      renameSync(logoFile.path, logoPath);
    }
    ...
    // step B: Subtitles
    if (assPath) {
      const escAssPath = assPath.replace(/\\/g, '/').replace(/:/g, '\\:');
      let assFilter = `ass=${escAssPath}`;
      if (fontsDir) {
        assFilter += `:fontsdir=${fontsDir.replace(/\\/g, '/').replace(/:/g, '\\:')}`;
      }
    ...
    let stderr = '';
    ff.stderr.on('data', (chunk: any) => { stderr += chunk.toString(); });

    ff.on('error', (err: any) => { console.error('ffmpeg spawn error', err); });
    ...
    readStream.on('close', () => {
      ...
      try { rmSync(tmpBase, { recursive: true, force: true }); } catch(e) { }
    });
  } catch (err) {
    try { rmSync(tmpBase, { recursive: true, force: true }); } catch(e) { }
