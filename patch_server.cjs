...
const newEndpoint = `
const srtToAss = require('./server/utils/srt-to-ass.js');
...
app.post('/api/video/export-capcut', upload.fields([
  ...
  const tmpBase = fs.mkdtempSync(path.join(os.tmpdir(), 'sda-export-capcut-'));
  try {
    ...
    if (subtitleFile) {
      ...
      fs.renameSync(subtitleFile.path, srtPath);
    ...
    readStream.on('close', () => {
      ...
      try { rimraf.sync(tmpBase); } catch(e) { }
    });
  } catch (err) {
    try { rimraf.sync(tmpBase); } catch(e) { }
